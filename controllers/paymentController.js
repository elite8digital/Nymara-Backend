...
export const createRazorpayOrder = async (req, res) => {
  try {
    ...
    for (let p of products) {
      ...
      let priceData;
      if (currency === "INR") {
        priceData = { amount: product.price, symbol: "₹" };
      } else {
        // Check if multi-currency pricing exists
        const currencyPrice = product.prices?.[currency];
        priceData = currencyPrice 
          ? { amount: currencyPrice.amount || currencyPrice, symbol }
          : { amount: product.price, symbol: "₹" };
      }
