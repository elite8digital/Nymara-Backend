...
export const forgetPassword = async (req, res) => {
  ...
  try {
    ...
    try {
      await axios.post(
        "https://lets-taxify.onrender.com/api/nymara/contact/forgot-password",
        {
          user: {
            name: user.name,
            email: user.email,
      ...
      console.log("📧 Forgot password email service called");
    } catch (emailErr) {
      console.error("⚠️ Email service failed:", emailErr.message);
      ...
      return res.status(500).json({
        success: false,
        message: "Email service failed. Try again.",
      });
    }
